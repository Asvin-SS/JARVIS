...
from PyQt6.QtWidgets import (
    ...
    QMainWindow, QMenu, QPushButton, QScrollArea, QSizePolicy, QSystemTrayIcon,
...
class LogWidget(QTextEdit):
    ...
    def append_log_no_type(self, text: str):
        """Append without typing animation (for streaming prefix)."""
        cur = self.textCursor()
        cur.movePosition(cur.MoveOperation.End)
        fmt = cur.charFormat()
...
class DashboardPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.lbl_status, self.lbl_mode = self._section_status("🤖", "Mark-XXXIX", "Loading model…")
        self.lbl_tasks = self._section("📋", "Active Tasks", "No active tasks.")
        ...
        self._task_tmr.timeout.connect(self.refresh_tasks)
        ...
        self._sh_tmr = QTimer(self)
        self._sh_tmr.timeout.connect(self.refresh_smart_home)
    ...
    def _section(self, icon: str, title: str, body: str) -> QLabel:
        card = QFrame()
        card.setStyleSheet(
            f"background: {C.DARK}; border: 1px solid {C.BORDER_B}; border-radius: 6px;"
        )
        cl = QVBoxLayout(card)
        ...
        body_lbl.setStyleSheet(f"color: {C.WHITE}; background: transparent; border: none;")
        cl.addWidget(body_lbl)
        self._lay.addWidget(card)
        return body_lbl

    def _section_status(self, icon: str, title: str, body: str):
        card = QFrame()
        card.setStyleSheet(
            f"background: {C.DARK}; border: 1px solid {C.BORDER_B}; border-radius: 6px;"
        )
        ...
        body_lbl = QLabel(body)
        body_lbl.setFont(QFont("Courier New", 8))
        body_lbl.setStyleSheet(f"color: {C.TEXT_MED}; background: transparent; border: none;")
        cl.addWidget(body_lbl)
        mode_lbl = QLabel("Idle")
        mode_lbl.setFont(QFont("Courier New", 7))
        mode_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent; border: none;")
        cl.addWidget(mode_lbl)
        self._lay.addWidget(card)
        return body_lbl, mode_lbl
    ...
    def set_mode(self, state: str):
        ...
        self.lbl_mode.setText(labels.get(state, state))
...
# ── SETUP OVERLAY ─────────────────────────────────────────────────────────────
# No API key needed. Just OS selection + Ollama health check.

class SetupOverlay(QWidget):
    done = pyqtSignal(str)   # emits os_name only
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl(
            "J.A.R.V.I.S runs fully local — no API keys needed.",
            9, color=C.PRI_DIM
        ))
        layout.addSpacing(6)
        ...
        # ── Ollama status ────────────────────────────────────────────
        layout.addWidget(_lbl("OLLAMA STATUS", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        self._ollama_lbl = QLabel("Checking…")
        self._ollama_lbl.setFont(QFont("Courier New", 9))
        self._ollama_lbl.setStyleSheet(f"color: {C.ACC2}; background: transparent;")
        layout.addWidget(self._ollama_lbl)
        self._check_ollama()
        ...
        layout.addWidget(_lbl("DEFAULT MODEL", 8, color=C.TEXT_DIM,
                               align=Qt.AlignmentFlag.AlignLeft))
        layout.addWidget(_lbl(
            "llama3.2  (fast · ~2 GB)  will be used by default.\n"
            "You can change this any time via ⚙ MODELS in the top bar.",
    ...
    def _check_ollama(self):
        def _probe():
            try:
                from llm_client import is_ollama_running
                if is_ollama_running():
                    self._ollama_lbl.setText("● Ollama detected  —  localhost:11434")
                    self._ollama_lbl.setStyleSheet(f"color: {C.GREEN}; background: transparent;")
                else:
                    self._ollama_lbl.setText(
                        "⊘ Ollama not found  —  install from ollama.com then run:  ollama serve"
                    )
                    self._ollama_lbl.setStyleSheet(f"color: {C.RED}; background: transparent;")
            except Exception:
                self._ollama_lbl.setText("⊘ Could not check Ollama status")
                self._ollama_lbl.setStyleSheet(f"color: {C.RED}; background: transparent;")
        threading.Thread(target=_probe, daemon=True).start()
...
class MainWindow(QMainWindow):
    ...
    window_shown_sig = pyqtSignal()
    window_hidden_sig = pyqtSignal()
    ...
    def __init__(self, face_path: str):
        ...
        body.addWidget(self._dash, stretch=0)

        self._dash_toggle = QPushButton("<")
        self._dash_toggle.setFixedSize(18, 48)
        self._dash_toggle.setCursor(Qt.CursorShape.PointingHandCursor)
        self._dash_toggle.setStyleSheet(
            f"QPushButton {{ background: {C.PANEL}; color: {C.PRI}; "
            f"border: 1px solid {C.BORDER}; border-left: none; }}"
        )
        self._dash_toggle.clicked.connect(self._toggle_dashboard)
        body.addWidget(self._dash_toggle, stretch=0)
    ...
    def _build_header(self) -> QWidget:
        ...
        try:
            ...
            settings_btn.setFont(QFont("Courier New", 8))
            settings_btn.setStyleSheet(
                f"background: transparent; color: {C.TEXT_MED}; "
            ...
            def _open_settings():
                ...
                dlg.exec()
                self._refresh_side()
            settings_btn.clicked.connect(_open_settings)
    ...
    def _toggle_mute(self):
        ...
        if self._muted:
            ...
            self._log.append_log("SYS: Microphone muted (TTS still in chat).")
        else:
    ...
    def hide_to_tray(self):
        self.hide()
        self.window_hidden_sig.emit()

    def _refresh_side(self):
        if hasattr(self, "_dash"):
            self._dash.refresh_all()

    def _setup_tray(self):
        self.tray = QSystemTrayIcon(self)
        face = str(BASE_DIR / "face.png")
        if os.path.exists(face):
            self.tray.setIcon(QIcon(face))
        else:
        ...
        menu.addAction("Hide", self.hide_to_tray)
        menu.addSeparator()
        menu.addAction("Quit", lambda: QApplication.instance().quit())
    ...
    def _check_config(self) -> bool:
        """Ready once os_system has been saved to settings.json."""
        try:
            from llm_client import get_settings
            s = get_settings()
            return bool(s.get("os_system"))
        except Exception:
            return False
    ...
    def _on_setup_done(self, os_name: str):
        """Save os_system into settings.json and close the overlay."""
        try:
            from llm_client import get_settings, save_settings
            settings = get_settings()
            settings["os_system"] = os_name
            save_settings(settings)
        except Exception as e:
            print(f"[JARVIS] Could not save settings: {e}")
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. Mark-3.9 online.")
        self._refresh_side()
...
class JarvisUI:
    def __init__(self, face_path: str, size=None):
        ...
        self.on_window_shown = None
        self.on_window_hidden = None
        self._win.window_shown_sig.connect(self._on_window_shown_internal)
        self._win.window_hidden_sig.connect(self._on_window_hidden_internal)

    def _on_window_shown_internal(self):
        if self.on_window_shown:
            self.on_window_shown()

    def _on_window_hidden_internal(self):
        if self.on_window_hidden:
    ...
    def write_log_no_type(self, text: str):
        self._win._log.append_log_no_type(text)

    def stream_chunk(self, chunk: str):
        self._win._log.stream_chunk(chunk)

    def set_muted(self, m: bool):
        if m != self._win._muted:
            self._win._toggle_mute()

    def wait_for_api_key(self):
        """Waits until first-run OS setup overlay completes."""
